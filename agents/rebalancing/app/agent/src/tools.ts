...
export const LLM_READ_TOOLS: ToolSet = {
  ...
  pcs_position: tool({
    description:
      "Read the WBNB/USDT(Venus) fee-100 pool tick and this address's PCS V3 NFT positions.",
    inputSchema: z.object({
      address: z.string().describe("0x wallet to inspect"),
    }),
    execute: async ({ address }) => {
      const { jsonSafe, readLpPositions, readPool, tokenBalances } =
        await import("@am-m/agent-strategy");
      const owner = address as `0x${string}`;
      const [pool, positions, balances] = await Promise.all([
        readPool(),
        readLpPositions(owner),
        tokenBalances(owner),
      ]);
      return jsonSafe({ pool, positions, balances });
    },
  }),
