 ...
 */
function nineRouterBaseUrl(llmCfg: TomlTable): string {
  const fromEnv = process.env.NINEROUTER_BASE_URL?.trim();
  const fromToml =
    typeof llmCfg.base_url === "string" ? llmCfg.base_url.trim() : "";
  const raw = fromEnv || fromToml || "http://127.0.0.1:20128/v1";
...
export function buildModel(): LanguageModel {
  ...
  const nineKey = process.env.NINEROUTER_API_KEY?.trim();
  if (nineKey && !process.env.OPENAI_API_KEY) {
    process.env.OPENAI_API_KEY = nineKey;
  }
  llmCfg.provider = "openai";
  llmCfg.base_url = nineRouterBaseUrl(llmCfg);
  if (!llmCfg.model) llmCfg.model = "kr/claude-sonnet-4.5";
