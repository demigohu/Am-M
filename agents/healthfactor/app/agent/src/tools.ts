...
export const LLM_READ_TOOLS: ToolSet = {
  ...
  venus_account: tool({
    description:
      "Read Venus testnet health factor, liquidity/shortfall, and vUSDT/vUSDC/vBNB balances for an address.",
    inputSchema: z.object({
      address: z.string().describe("0x wallet to inspect"),
    }),
    execute: async ({ address }) => {
      const { jsonSafe, readVenusAccount } = await import("@am-m/agent-strategy");
      return jsonSafe(await readVenusAccount(address as `0x${string}`));
    },
  }),
