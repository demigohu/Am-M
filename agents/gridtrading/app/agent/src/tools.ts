...
export const LLM_READ_TOOLS: ToolSet = {
  ...
  pcs_book: tool({
    description:
      "Read the WBNB/USDT(Venus) fee-100 pool tick and wallet WBNB/USDT balances for grid trading.",
    inputSchema: z.object({
      address: z.string().describe("0x wallet to inspect"),
    }),
    execute: async ({ address }) => {
      const { jsonSafe, readPool, tokenBalances } = await import(
        "@am-m/agent-strategy"
      );
