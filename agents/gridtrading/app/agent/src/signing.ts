...
import { createClient, BNB_TESTNET, type Session } from "@altananetwork/sdk";
import type {
  ExecuteFn,
  ExecuteResultLike,
...
/**
 * DeFi writes over a USER session. Never an LLM tool. Never the agent
 * admin keystore — the process only holds the session object.
 */
export const executeSessionCalls: ExecuteFn = async (
  session,
  calls,
  label,
  ...
  console.log(
    `[signing] ${label} ${result.status} ${result.transactionHash ?? result.callsId ?? ""}`,
  );
  return result;
};
